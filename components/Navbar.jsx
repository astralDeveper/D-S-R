    ...
    {/* dekstop navigation */}
        ...
        <nav className='shadow-[0_11px_6.5px_rgba(0,0,0,0.25)]'>
            ...
            <ul className='flex items-center gap-14 text-xl font-medium max-xl:gap-8 max-xl:text-lg max-lg:hidden'>
                <Link href={'/'}><li className={`cursor-pointer hover:text-[#000000af] transition-all ${isActive('/')}`}>Home</li>  {router === '/' && <hr className='border-2 border-[#E51A1A]' />}</Link>
        ...
        <div 
                className={`fixed top-24 max-lg:top-20 max-sm:top-16 left-0 w-full bg-gray-100 p-4 py-8 transition-all duration-300 ease-in-out ${mobileNav ? 'max-h-screen opacity-100 translate-y-0' : 'max-h-0 opacity-0 -translate-y-4 overflow-hidden'}`}
